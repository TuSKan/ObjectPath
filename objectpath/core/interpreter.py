...
def map_args(function, iterable, fargs=[]):
    if len(fargs) > 0:
        iterable = list(iterable) if not isinstance(
            iterable, list) else iterable
        iterable_args = [list(args) for args in product(iterable, fargs)]
    else:
        iterable_args = [[x] for x in iterable]
    return map(function, iterable_args)
    ...
    def execute(self, expr):
        ...
        def exe(node):
            ...
            if op == "or":
                ...
                if len(node) > 2:
                    ...
                    if typefst is list and typesnd is list:
                        if D:
                            self.debug(
                                "both sides are lists, returning '%s'", fst + snd)
                        return fst + snd
                    ...
                    if typefst in STR_TYPES or typesnd in STR_TYPES:
                        if D:
                            self.info(
                                "doing string comparison '%s' is '%s'", fst, snd)
                    ...
                    if D:
                        self.debug(
                            "standard addition, returning '%s'", fst + snd)
                    return fst + snd
                else:
                    return exe(node[1])
            elif op == "-":
            ...
            # 		return fst <= snd and snd or False
                ...
                typesnd = type(snd)
                if D:
                    self.debug("type fst: '%s', type snd: '%s'",
                               typefst, typesnd)
                if typefst in STR_TYPES:
                    ...
                    if D:
                        self.info(
                            "doing float comparison '%s is %s'", fst, snd)
                    try:
                    ...
                    if D:
                        self.info(
                            "doing integer comparison '%s is %s'", fst, snd)
                    try:
                        ret = int(fst) == int(snd)
                    except:
                        ret = False
                    ...
                    if D:
                        self.info(
                            "doing array comparison '%s' is '%s'", fst, snd)
                    ret = fst == snd
                elif typefst is dict and typesnd is dict:
                    if D:
                        self.info(
                            "doing object comparison '%s' is '%s'", fst, snd)
                    ret = fst == snd
                    ...
                    if fst is None and snd is None:
                        ...
                        if D:
                            self.info(
                                "doing None comparison %s is %s = %s", color.bold(
                                    fst), color.bold(snd),
                ...
                if op == "is not":
                    ...
                    if D:
                        self.info("returning %s is %s => %s", color.bold(self.cleanOutput(
                            fst)), color.bold(self.cleanOutput(snd)), color.bold(ret))
                    return ret
            elif op == "re":
                ...
                if type(fst) not in STR_TYPES+[RE_TYPE]:
                    raise Exception("operator " + color.bold("matches") +
            ...
            # 	return self.node
                ...
                snd = exe(node[2])
                if D:
                    self.debug(color.op(".") + " right is '%s'",
                               color.bold(snd))
                if typefst in ITER_TYPES:
                    if D:
                        self.debug(
                            color.op(
                                ".") + " filtering %s by %s", color.bold(self.cleanOutput(fst)),
                            color.bold(snd)
                        )
                    if type(snd) in ITER_TYPES:
                ...
                if D:
                    self.debug(
                        color.op("..") +
                        " finding all %s in %s", color.bold(snd),
                        color.bold(self.cleanOutput(fst))
                    )
                if type(snd) in ITER_TYPES:
                    ...
                    if D:
                        self.debug(color.op("..") + " returning %s",
                ...
                if len_node is 1:  # empty list
                    ...
                    return []
                # list - preserved to catch possible event of leaving it as '[' operator
                if len_node is 2:
                    if D:
                ...
                if len_node is 3:  # selector used []
                    ...
                    if selectorIsTuple and selector[0] is "[":
                        ...
                        for i in fst:
                            if D:
                                self.debug(
                                    "setting self.current to %s", color.bold(i))
                            self.current = i
                            nodeList_append(
                                exe((selector[0], exe(
                                    selector[1]), exe(selector[2])))
                            )
                        if D:
                            self.debug(
                                "returning %s objects: %s", color.bold(
                                    len(nodeList)),
                                color.bold(nodeList)
                            )
                        return nodeList
                    ...
                    if selectorIsTuple and selector[0] in SELECTOR_OPS:
                        if D:
                            self.debug(
                                "found %s operator in selector, %s", color.bold(
                                    selector[0]),
                                color.bold(selector)
                            )
                        if type(fst) is dict:
                        ...
                        if type(selector[1]) is tuple and selector[1][0] == "name":
                            selector = (
                                selector[0], selector[1][1], selector[2])
                    ...
                    if typefst in [tuple] + ITER_TYPES + STR_TYPES:
                        ...
                        if typesnd in NUM_TYPES or typesnd is str and snd.isdigit():
                            ...
                            if D:
                                self.info(
                                    "getting %sth element from '%s'", color.bold(
                                        n),
                                    color.bold(fst)
                                )
                ...
                try:
                    ...
                    if D:
                        self.debug(
                            "NOT ERROR: can't map '%s' with '%s'", node[2:], exe)
                # arithmetic
                if fnName == "sum":
                    args = args[0]
                ...
                # 	return re.sub(args[1],args[2],args[0])
                    ...
                    return chain(*map_args(lambda x: exe(("fn", args[0], x)), args[1], fargs=args[2:]))
                ...
                # time
                    ...
                    if fnName == "dateTime":
                        if type(args[0]) in ITER_TYPES:
                            return list(map_args(timeutils.dateTime, args[0], args[1:]))
                        return timeutils.dateTime(args)
                    # TODO move lang to localize() entirely!
                    if fnName == "age":
                        if type(args[0]) in ITER_TYPES:
                            return list(map_args(timeutils.age, args[0], args[1:]))
                        return timeutils.age(args)
                    ...
                    if not calendar:
                        import calendar
                    return int(
                        calendar.timegm(args.timetuple()) *
                        1000 + args.microsecond/1000
                    )
                ...
                # polygons
                    ...
                    return 0.5*abs(
                        sum(x0*y1 - x1*y0 for ((x0, y0), (x1, y1))
                            in segments(args[0]))
                    )
                # misc
                elif fnName == "keys":
